...
class TaskBarIcon(wx.TaskBarIcon):
    ...
    def CreatePopupMenu(self):
        ...
        create_menu_item(menu, 'Publish screenshot', self.publish_screenshot)
    ...
    def send_screenshot(self, event):
        ...
        params = {'image': open(t+'.png','rb'), 'lesson': settings['id'], 'publish': 'no'}
    ...
    def publish_screenshot(self, event):
        ...
        opener = streaminghttp.register_openers()
        params = {'image': open(t+'.png','rb'), 'lesson': settings['id'], 'publish': 'yes'}
        datagen, headers = multipart_encode(params)
        response = opener.open(urllib2.Request(settings['url2'], datagen, headers))
